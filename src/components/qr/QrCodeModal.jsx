...
export function QrCodeModal({ isOpen, onClose, productId, productName }) {
  ...
  // E.g. http://localhost:5173/track/123
  const url = `${window.location.origin}/track/${productId}`;
  ...
  const downloadQR = () => {
    ...
    img.onload = () => {
      ...
      downloadLink.download = `QR_${productName.replace(/\s+/g, '_')}.png`;
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-800 border-border/60 rounded-xl shadow-2xl max-w-sm w-full border overflow-hidden">
        <div className="bg-gradient-to-r from-slate-800 to-slate-700 p-4 border-b border-border/60 flex justify-between items-center">
          <h3 className="font-semibold text-lg text-foreground">Product QR Code</h3>
        ...
        <div className="p-6 flex flex-col items-center space-y-6">
          ...
          <div className="text-center space-y-2">
            <h4 className="font-semibold text-foreground text-lg">{productName}</h4>
            <p className="text-sm text-muted-foreground break-all px-2">
              Consumers can scan this to track authenticity and supply chain journey.
            </p>
          </div>
